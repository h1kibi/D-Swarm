...
startup and rewritten on each mutation. It answers which engines are eligible
for automatic dispatch, with optional per-category overrides for direction
profiles and endpoint-backed Pi workers.

The dispatch path (apps/web/drivers.py) reads `resolve(category)` as the FALLBACK
when the request body carries no explicit engines. Worker capacity is derived
from each selected profile's `max_running`; there is no separate bootstrap-count
knob.
"""
...
def _direction_profiles() -> list[dict[str, object]]:
    ...
    for direction in DIRECTIONS:
        out.append({
            ...
            "max_running": 2, "max_review_running": 1,
...
DEFAULT_WORKER_PROFILES = [
    {"id": "pi-worker", "name": "pi-worker",
     ...
     "max_running": 3, "max_review_running": 1, "priority": 10,
...
def _ordinary_worker_roles(profile: dict[str, Any]) -> set[str]:
    ...
    return {
        str(r)
        for r in roles
        if str(r) in {"bootstrap", "explore", "respond"}
    }
...
class WorkerConfigStore:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            self._data = {}
        # Retired execution/config paths are intentionally discarded at the
        # persistence boundary. Historical files are readable, but their fields
        # never re-enter the live config or new output.
        self._data.pop("stage_policy", None)
        self._data.pop("start_workers", None)
        raw_overrides = self._data.get("overrides")
        if isinstance(raw_overrides, dict):
            for override in raw_overrides.values():
                if isinstance(override, dict):
                    override.pop("start_workers", None)
    ...
    def _project_identity_to_legacy(self) -> None:
        ...
        try:
            ...
            # The dispatch roster MUST track the seats' enabled toggles — that's
            # the only roster control the seat UI exposes. A stale top-level
            ...
            # the roster is exactly the enabled seats, preserving the order of any
    ...
    def get(self) -> dict[str, Any]:
        ...
        review_policy = self._clean_review_policy(d.get("review_policy"))
        names = {str(p.get("name") or p.get("id")) for p in worker_profiles}
        review_engine = _remap_profile_ref(
            review_policy.get("engine") or DEFAULT_REVIEW_POLICY["engine"],
            worker_profiles,
            worker_backend,
        )
        ...
        review_policy["engine"] = review_engine
        ...
        if isinstance(raw_ov, dict):
            for cat, ov in raw_ov.items():
                ...
                if not cat_engines:
                    continue
                overrides[str(cat)] = {"engines": cat_engines}
        result = {
            "engines": engines,
    ...
    def resolve(self, category: Optional[str]) -> dict[str, Any]:
        ...
        if ov:
            ...
            # profile), which lets the scheduler repeatedly try to dispatch past
    ...
    def set(
        ...
        *,
        ...
        if review_policy is not None:
            profiles_for_review = self._clean_worker_profiles(
                worker_profiles if worker_profiles is not None else self._data.get("worker_profiles"))
            clean_review = self._clean_review_policy(review_policy)
            clean_review["engine"] = _remap_profile_ref(
                clean_review.get("engine") or DEFAULT_REVIEW_POLICY["engine"],
                profiles_for_review,
                target_backend,
            )
            self._data["review_policy"] = clean_review
        ...
        if overrides is not None:
            ...
            for cat, ov in overrides.items():
                ...
                clean_ov[str(cat)] = {"engines": cat_engines}
        ...
        # dispatch roster could have changed — i.e. worker_profiles or engines were
    ...
    def _sync_worker_counts(self, *, link_profile_capacity: bool) -> None:
        ...
        # is what ballooned a stale single-seat roster up to max_workers and made
        ...
        # Historical start_workers was a separate bootstrap knob. Capacity is
        # now entirely owned by profile max_running and scheduler policy.
        self._data.pop("start_workers", None)
    ...
    def set_runtime_environment(self, *, backend: str, runtime_id: str) -> dict[str, Any]:
        ...
        review = self._clean_review_policy(self._data.get("review_policy"))
        review["engine"] = rewrite_ref(
            review.get("engine") or DEFAULT_REVIEW_POLICY["engine"])
        self._data["review_policy"] = review
    ...
    def _clean_review_policy(value: Any) -> dict[str, Any]:
        """Normalize Reason review settings without the retired stage wrapper."""
        raw = value if isinstance(value, dict) else {}
        review = dict(DEFAULT_REVIEW_POLICY)
        review["enabled"] = bool(raw.get("enabled", review["enabled"]))
        review["engine"] = str(raw.get("engine") or review["engine"]).strip()
        for key in ("provider_ref", "credential_source", "credential_account", "base_url", "wire_api", "model"):
            if raw.get(key) is not None:
                review[key] = str(raw.get(key) or "").strip()
        ...
        for key in (
            ...
            if raw.get(key) is not None:
                review[key] = WorkerConfigStore._coerce_nonneg_int(raw.get(key), int(review[key]))
        for key in (
            "on_course_correct", "on_reason_dry", "on_candidate_spike",
            "on_operator_hint", "allow_review_fallback",
        ):
            if raw.get(key) is not None:
                review[key] = bool(raw.get(key))
        return review
