...
class Intent:
    ...
    dup_of: str = ""
    reopen_because: str = ""
    # Reason-centered dispatch fields. Empty profile means the scheduler picks
    # the challenge category profile; mode can request recon/explore/review.
    profile: str = ""
    # Direction (web/pwn/rev/crypto/misc/forensics/aisec) selects the worker
    # profile/image a composite challenge should use for THIS intent. Empty
    # means "use the challenge category's primary direction profile".
    direction: str = ""
    mode: str = ""
    surface_target: str = ""
    priority: float = 0.5
  ...
  "drift": "<only if verdict=course_correct: what's going wrong + the correct direction>",
	  "intents": [
	    {"id": "I1", "from": [3, 7], "goal": "<one concrete, independent next direction>",
	     ...
	     "reopen_because": "", "profile": "exploit", "mode": "explore",
	     "direction": "web",
	     "surface_target": "", "priority": 0.5, "requires_recon": false,
	     "host_scan": false}
	  ],
	  "pinned_facts": [3, 7],
...
- When the board exposes a NEW_SURFACE or a new unknown backend/target, emit an
  intent with "mode": "recon", "profile": "recon", and "surface_target" set to
  the exact target. When you want a focused attack, use "mode": "explore" and
  "profile": "exploit".
- "direction" chooses WHICH direction worker profile this intent should run on:
  web / pwn / rev / crypto / misc / forensics / aisec. On a composite challenge
  (e.g. a web app whose flag needs a crypto step), set each intent to the
  direction that actually does the work. Use "" (empty) or "unclear" when the
  direction is the challenge's own primary category — the scheduler then falls
  back to the category's primary direction profile.
- In CTF web mode, default to web-application discovery/exploitation on the
  supplied HTTP(S) surface. Do NOT propose broad host/port scanning or non-web
  service exploitation (SSH/SMB/Redis/MySQL/RPC/etc.) just because an IP exists.
  Use "host_scan": true ONLY when the challenge text, operator guidance, or
  verified web output explicitly points to another port/service/internal address;
  then keep the goal to one targeted discovery/proof and route follow-up work back
  through the web attack path.
...
def parse_reason_reply(text: str, *, max_intents: int = 4) -> ReasonResult:
    ...
    for i, raw in enumerate(d.get("intents", [])[:max_intents]):
        ...
        intents.append(Intent(
            ...
            requires_recon=_as_bool(raw.get("requires_recon"), False),
            task_kind=str(raw.get("task_kind") or "").strip(),
            host_scan=_as_bool(raw.get("host_scan"), False),
        ))
