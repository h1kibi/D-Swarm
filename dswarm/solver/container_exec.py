...
to the supervisor over a per-run Unix domain socket (the `rcp` backend, default).
  ...
  - ONE container per RUN (`dswarm-run-<safe_run_id>`), from the tool image. In rcp
...
for control. Kept behind DSWARM_WORKER_BACKEND=container_dockerexec as an emergency
...
# single explicit DSWARM_WORKER_IMAGE keeps the historical one-image behavior;
# otherwise the challenge category resolves to the same unified image. Per-category
# env overrides win:
...
def worker_image_for_category(category: str) -> str:
    ...
    Precedence: DSWARM_WORKER_IMAGE (explicit single image) > per-category env
    override > the per-category pi image > the default worker image.
    """
    explicit = os.environ.get("DSWARM_WORKER_IMAGE", "").strip()
    if explicit:
        return explicit
    cat = (category or "").strip().lower()
    per_cat = os.environ.get(f"DSWARM_CATEGORY_IMAGE_{cat.upper()}", "").strip()
    if per_cat:
        return per_cat
    return _CATEGORY_IMAGES.get(cat, WORKER_IMAGE)


def worker_image_for_profile(profile: Optional[dict[str, Any]] = None,
                             *, category: str = "") -> str:
    """The worker image for one agent/profile.

    Precedence:
      1. DSWARM_WORKER_IMAGE (explicit single image)
      2. DSWARM_AGENT_IMAGE_<PROFILE_NAME> (per-agent env override)
      3. profile.image
      4. per-category env override / category default
      5. WORKER_IMAGE
    """
    explicit = os.environ.get("DSWARM_WORKER_IMAGE", "").strip()
    if explicit:
    ...
    if profile:
        ...
        if name:
            env_name = f"DSWARM_AGENT_IMAGE_{name.upper().replace('-', '_')}"
            env_img = os.environ.get(env_name, "").strip()
            if env_img:
                return env_img
        profile_img = str(profile.get("image") or "").strip()
        if profile_img:
            return profile_img
    return worker_image_for_category(category)
...
#   DSWARM_HOST_DATA_ROOT      — the host's real path (e.g. /opt/dswarm/data)
#   DSWARM_CONTAINER_DATA_ROOT — where it's mounted in the web container
#                                (default: same as host root → identity mirror)
# Unset (bare host) → identity, no translation.
_HOST_DATA_ROOT = (os.environ.get("DSWARM_HOST_DATA_ROOT") or "").strip()
_CONTAINER_DATA_ROOT = (os.environ.get("DSWARM_CONTAINER_DATA_ROOT") or _HOST_DATA_ROOT).strip()
...
_WORKER_USER = (os.environ.get("DSWARM_WORKER_USER") or "kali").strip() or "kali"
...
def _fallback_worker_uid_gid() -> tuple[int, int]:
    uid = _env_int("DSWARM_WORKER_UID")
    gid = _env_int("DSWARM_WORKER_GID")
    return (
...
def _query_worker_uid_gid(image: str) -> tuple[int, int]:
    ...
    if len(vals) != 2:
        return _fallback_worker_uid_gid()
    uid_override = _env_int("DSWARM_WORKER_UID")
    gid_override = _env_int("DSWARM_WORKER_GID")
    return (
...
def _mount_source(path: str) -> str:
    ...
    should bind-mount. Identity unless DSWARM_HOST_DATA_ROOT is set AND `path` is
    under DSWARM_CONTAINER_DATA_ROOT (the mirrored data volume)."""
...
class ContainerHandle:
    ...
    control_dir: Optional[str] = None   # host dir bind-mounted to /run/dswarm/control (carries the token)
...
def ensure_container(run_id: str, host_workspace: str, *,
    ...
    if account_root:
        ...
        from dswarm.solver.credential_accounts import project_account_root
        projection = os.path.join(host_workspace, ".dswarm_accounts")
        try:
    ...
    # receiver by the web service's network alias (DSWARM_CONTROL_HOST=web), which
    # only resolves if the worker joins the SAME compose network. DSWARM_WORKER_NETWORK
    # names that network (e.g. "dswarm_net"); when set it overrides the per-profile
    ...
    _net_override = (os.environ.get("DSWARM_WORKER_NETWORK") or "").strip()
    ...
    if mode == "rcp":
        ...
        control_dir = os.path.join(host_workspace, ".dswarm_control")
    ...
    with _ENSURE_LOCK:
        ...
        if mode == "dockerexec":
            ...
            from dswarm.solver.control_receiver import (
...
def _await_supervisor(handle: ContainerHandle) -> None:
    ...
    if handle.mode != "rcp":
        return
    from dswarm.solver.control_client import wait_supervisor_ready
...
def teardown_container(run_id: str, *, remove: bool = True) -> None:
    ...
    name = _run_container_name(run_id)
    _docker("rm", "-f", name, timeout=20)
    # legacy sweep: old design left dswarm-w-<safe>-<uuid> containers; clean any.
    safe = _safe(run_id)
    r = _docker("ps", "-aq", "--filter", f"name=dswarm-w-{safe}-", timeout=15)
...
def _recover_run_container(handle: ContainerHandle) -> None:
    ...
    try:
        ...
        pass
    name = _run_container_name(handle.run_id)
    try:
        _docker("rm", "-f", name, timeout=20)
    except Exception:
        pass
...
def _run_rcp_with_recover(fn, *args, handle: ContainerHandle, **kwargs) -> "CliResult":
    """Call an rcp transport function; on a dead supervisor/link (ControlError),
    recreate the run container (fresh supervisor) and retry ONCE. The retry's
    `_resolve_link` waits for the new supervisor to dial back in."""
    from dswarm.solver.control_client import ControlError
    try:
        return fn(*args, **kwargs)
    except ControlError:
        _recover_run_container(handle)
        return fn(*args, **kwargs)
...
def run_cli_container(driver: CliDriver, argv: list[str], *, handle: ContainerHandle,
    ...
    if handle.mode == "rcp":
        from dswarm.solver.control_client import run_cli_rcp
        ...
        res = _run_rcp_with_recover(
            run_cli_rcp, driver, cont_argv, handle=handle,
            run_id=handle.run_id, container_cwd=cont_cwd, timeout=timeout, env=env)
...
def run_cli_streaming_container(
    ...
    cont_cwd = handle.to_container_cwd(cwd)
    if handle.mode == "rcp":
        from dswarm.solver.control_client import run_cli_streaming_rcp
        cont_argv = _containerize_argv(driver.name, argv)
        tag = uuid.uuid4().hex[:12]
        rec = _RUNTIME_REGISTRY.create(handle=handle, tag=tag, driver=driver.name,
                                       cwd=cont_cwd, argv=cont_argv)
        ...
        res = _run_rcp_with_recover(
            run_cli_streaming_rcp, driver, cont_argv, handle=handle,
            run_id=handle.run_id, container_cwd=cont_cwd, timeout=timeout,
            on_step=on_step, env=env,
...
# Kept behind DSWARM_WORKER_BACKEND=container_dockerexec. The host shells `docker
# exec` per worker into a `sleep infinity` container; per-worker control is
# `pkill -f dswarm_wtag_<tag>` inside the container. To be removed once rcp settles.

class _ContainerProc:
    """Wraps ONE worker's `docker exec`. Control maps to `pkill -<SIG> -f
    DSWARM_WTAG=<tag>` INSIDE the container, so kill/pause/resume hit only this
    ...
    def _pkill(self, sig: str) -> None:
        _docker("exec", self.container, "pkill", sig, "-f", f"dswarm_wtag_{self.tag}",
                timeout=15)
...
class _DockerExecBackend:
    ...
    def _exec_argv(handle: ContainerHandle, argv: list[str], *, container_cwd: str,
        ...
        - DSWARM_WTAG env + the `dswarm_wtag_<tag>` sentinel `$0` let `pkill -f <tag>`
          target ONLY this worker's tree for per-worker kill/pause.
        """
        argv = _containerize_argv(driver_name, argv)
        cmd = ["docker", "exec", "-w", container_cwd, "-e", f"DSWARM_WTAG={tag}"]
        if env:
            for k, v in env.items():
                ...
                if k == "PI_CODING_AGENT_DIR":
                    ...
                    continue
                if k.startswith((
                    "DSWARM_", "ANTHROPIC_", "OPENAI_", "DEEPSEEK_"
                )):
                    cmd += ["-e", f"{k}={v}"]
        ...
        cmd += ["sh", "-c", inner, f"dswarm_wtag_{tag}"]
        return cmd
    ...
    def run(driver: CliDriver, argv: list[str], *, handle: ContainerHandle,
        ...
        try:
            ...
            _docker("exec", handle.container, "pkill", "-KILL", "-f", f"dswarm_wtag_{tag}",
                    timeout=15)
