...
def _seat_direction_image(seat: dict[str, Any]) -> str:
    """The worker image a Seat maps to in the flat legacy profile.
    ...
    """
    for key in ("label", "id"):
        name = str(seat.get(key) or "").strip().lower()
        if not name.startswith("pi-"):
            continue
        direction = name[3:].strip()
