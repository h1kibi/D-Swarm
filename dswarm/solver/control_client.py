...
from dswarm.solver.control_receiver import ControlError, ControlReceiver, _SupervisorLink
...
# prelude): the engine credential vars + our own DSWARM_* knobs. HOME is special-
...
def _filter_env(env: Optional[dict]) -> dict[str, str]:
    """Same selection the old docker-exec path did: only credential/DSWARM vars, plus
...
def _resolve_link(run_id: str, *, deadline_s: "Optional[float]" = None) -> _SupervisorLink:
    ...
    Default: DSWARM_CONTROL_LINK_DEADLINE (40s; raise to 120 on Docker Desktop)."""
