...
`dswarm/core/llm.py`. This module adds a *local-only* convenience: if a `.env`
...
- Real environment variables WIN: `load_env()` never overrides a var already set
  in the shell (`override=False`), so `DSWARM_DEEPSEEK_API_KEY=... uv run ...`
...
# repo root = three levels up from this file (dswarm/core/dotenv_boot.py)
