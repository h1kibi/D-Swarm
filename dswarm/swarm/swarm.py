...
from dswarm.swarm.shared_graph import SharedGraph, SQLiteSharedGraph, canonicalize_lane
...
class Swarm:
    ...
    def __init__(
        ...
        *,
        ...
        self.reason_swarm = bool(reason_swarm)
    ...
    async def _reconcile_blackboard_skill(self) -> None:
        ...
        if self.worker_backend == "container":
            return
        try:
            from dswarm.solver.cli_solver import sync_deployed_blackboard_skills
    ...
    async def _run_race(self) -> SwarmOutcome:
        ...
        try:
            ...
            if hitl_task is not None:
                hitl_task.cancel()
                await asyncio.gather(hitl_task, return_exceptions=True)
            # Race mode historically closed the DB without running the same
            # finalize sweep as coordinator mode. That leaked operator-generated
            # active intents after a solved race and made a finished run look
            # dispatchable on the next observer pass. Apply the append-only
            # lifecycle transition before the graph is closed below.
            if self.shared_graph is not None:
                try:
                    race_reason = (
                        "solved" if winner is not None or self._flags_complete()
                        else ("operator_stop" if self._operator_stop else "runtime_failure")
                    )
                    fin = self.shared_graph.release_claims_for_finalize(
                        reason=race_reason)
                    await self._emit_finalize_lifecycle_deltas(fin, race_reason)
                    await self._drain_graph_to_bus(emit_bb=self._emit_bb_bus)
                except Exception:
                    # Finalization is best-effort here, but must never mask the
                    # worker outcome or prevent subprocess cleanup.
                    pass
        ...
        if winner is not None:
            ...
            if self.knowledge is not None:
                from dswarm.learning.distill import (
    ...
    def _probe_engine_health(self, name: str, role: str) -> "tuple[bool, str]":
        ...
        cache around it."""
        # A container-backed worker is not represented by a CLI on the
        # coordinator host. This is true both when the coordinator itself runs in
        # the web container and when the desktop app runs on Windows/macOS/Linux
        # and launches the worker in Docker. Probing the host in that case gives a
        # false negative (missing binary, wrong HOME, or wrong credentials), which
        # drops the entire roster before the first worker can be spawned. The
        # worker container performs the real auth/liveness check when it starts.
        from dswarm.core.runtime_env import is_web_container
        from dswarm.solver.cli_driver import driver_for
        try:
            profile = self._profile_for_engine(name, role=role, advance=False)
    ...
    def _runtime_env_for(
        ...
        *,
        ...
        if profile:
            ...
            env["DSWARM_CREDENTIAL_ACCOUNT_ID"] = profile["credential_account"]
        ...
        if self.worker_root is not None and container is not None:
            ...
            try:
                from dswarm.solver.cli_solver import materialize_runtime_blackboard_skill
    ...
    async def _run_coordinator(self) -> SwarmOutcome:
        ...
        async def _worker_unavailable() -> SwarmOutcome:
            if hitl_task is not None:
                hitl_task.cancel()
                await asyncio.gather(hitl_task, return_exceptions=True)
            await self._finalize_coordinator_run(
        ...
        try:
            ...
            return await _worker_unavailable()
        await self._emit_coord_bb(
            "worker_health_check",
            phase="completed" if healthy else "failed",
            role="bootstrap",
        ...
        # original dswarm race). SLOW PATH: no flag → their facts are on the shared
        ...
        try:
            while tasks:
                ...
                while open_intents and spawned_this_round < self.explore_spawn_batch:
                    ...
                    try:
                        preferred = self._profile_for_direction(
                            str(intent.get("direction") or ""))
