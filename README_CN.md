...
**Project D-Swarm** 是一个自主攻防安全多 Agent 蜂群：给它一道 CTF 题目或一个书面授权的渗透测试目标，
它会派出一群完整的 CLI 编码智能体（当前引擎为 `pi`）在容器沙箱里并行进攻。所有 worker 在一张
**append-only 的共享证据图（黑板）** 上协作——谁发现的事实大家都能用，谁走过的死路大家都不再试；
...
**身份声明**：本项目 fork 自 [FishCodeTech/muteki](https://github.com/FishCodeTech/muteki)
（AGPL-3.0 已保留），现在是独立产品——与上游无共同提交历史；截至 2026-08-27 的全树对比显示：
共享证据图从 584 行长到 5153 行、263 个文件为本仓独有、内核仅个位数文件字节级一致。
归属与分叉数据见 [NOTICE](NOTICE) 与
[docs/00-architecture-spec.md](docs/00-architecture-spec.md#1-定位与边界)（架构规范）、
[docs/10](docs/10-v4-kernel-improvement-implementation.md)（实施账本）。
