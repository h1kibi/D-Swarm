...
> **2026-08-30 可观测补强（M9a 栈实测后追加）**：对真实运行验收时发现四处比 P0 更深
> 的观测缺口，已全部落地——运行时卡片（池生命周期/探测/迁移历史，`38b8556`）、Reason
> 规划循环条（停止原因/侦察摘要/周期耗时，`e1269f7`）、账本冲突归因（`c9f93a7`）、
> stage×status 双维正交状态（`15fdeaf`）。账本完整性前置修复见 `a6055ab`。每项均带
> 确定性测试并用真实运行数据浏览器验收。
