...
import dswarm.core.dotenv_boot as boot
...
def test_loads_values(tmp_path, monkeypatch):
    m = _fresh()
    env = tmp_path / ".env"
    env.write_text("DSWARM_TEST_FOO=from_file\n")
    monkeypatch.delenv("DSWARM_TEST_FOO", raising=False)
    assert m.load_env(env) is True
    import os
    assert os.environ.get("DSWARM_TEST_FOO") == "from_file"
...
def test_shell_env_wins(tmp_path, monkeypatch):
    ...
    m = _fresh()
    env = tmp_path / ".env"
    env.write_text("DSWARM_TEST_BAR=from_file\n")
    monkeypatch.setenv("DSWARM_TEST_BAR", "from_shell")
    m.load_env(env)
    import os
    assert os.environ.get("DSWARM_TEST_BAR") == "from_shell"
...
def test_idempotent_latch(tmp_path):
    ...
    m = _fresh()
    env = tmp_path / ".env"
    env.write_text("DSWARM_TEST_BAZ=1\n")
    assert m.load_env(env) is True
